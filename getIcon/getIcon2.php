<?php
	...
	$subcomponentValue.=",'inner_collar_band','top_cuff_shell','bottom_cuff_shell','cuff_piping'";
	$subcomponentValue.=",'top_button_placket','top_buttonhole_placket','pocket','full_sleeve'";
	...
	setHandle('fullSleeve');
	//setHandle('halfSleeve');

	$fullSleeveHandle= realpath('fullSleeve');
	//$halfSleeveHandle= realpath('halfSleeve');

	$fabricName= $argv[1];

	$cmn= 'convert -resize 610x610 xc:none '.$fullSleeveHandle.PRT.$fabricName;
	...
	foreach($arrTable as $tableRow)
	{
		...
		composeOverlayBackground($fabricHandle.PRT.$fabricName, 'fullSleeve'.PRT.$fabricName);
	}

	$collarButtonsHandle= $handle610.PRT.'COL003'.PRT.'front'.PRT.'outer_collar_band'.PRT.'button';

	composeOverlayBackground($collarButtonsHandle.PRT.'BTN5.png', 'fullSleeve'.PRT.$fabricName);

	$placketButtonsHandle= $handle610.PRT.'PLA002_2'.PRT.'front'.PRT.'top_buttonhole_placket'.PRT.'button';

	composeOverlayBackground($placketButtonsHandle.PRT.'BTN5.png', 'fullSleeve'.PRT.$fabricName);

	$fabricNameWithoutVersion= getFabricNameWithoutVersion($fabricName);

	rename('fullSleeve'.PRT.$fabricName, 'fullSleeve'.PRT.$fabricNameWithoutVersion);
	...
	function getComponentName($componentRowValue)
	{
		switch($componentRowValue)
		{
			...
			case '_sleeve':
								$componentValue= 'SLE001_1';
								break;

			case '_placket':
								$componentValue= 'PLA001_2';
								break;
